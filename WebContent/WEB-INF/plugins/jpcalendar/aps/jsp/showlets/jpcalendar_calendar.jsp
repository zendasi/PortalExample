...
<%@ taglib uri="/WEB-INF/plugins/jpcalendar/aps/tld/jpcalendar-aps-core.tld" prefix="cal" %>
...
<c:forEach var="settimana" items="${jpcalendarCal.calendario}" varStatus="status">
	...
	<c:forEach var="giorno" items="${settimana}" varStatus="dayStatus">
		...
		<c:if test="${giorno.hasEvents}"><a href="<wp:url page="listEvents" ><wp:parameter name="selectedDate" ><c:out value="${giorno.formattedDate}"/></wp:parameter></wp:url>"><c:if test="${giorno.formattedDate == today}"><strong></c:if><c:out value="${giorno.day}"/><c:if test="${giorno.formattedDate == today}"></strong></c:if></a></c:if>
		</td>
		</c:if>
