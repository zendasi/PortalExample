...
public class EntityHandler extends DefaultHandler {
	...
	public EntityHandler getHandlerPrototype() {
		...
		try {
			...
			throw new RuntimeException("Error while cloning the entity handler: class " + this.getClass().getName());
		}
		return handler;
	}
	...
	/**
	 * Overrides the method in the ContentHanler.
	...
	public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException{
		...
		try {
			if (this.isIntoAttributes()) {
				...
				} else this.startEntityElement(uri, localName, qName, attributes);
			}
		} catch (Throwable t) {
	...
	/**
	 * Overwrite the method in the ContentHandler interface.
		...
		try {
			if (this.isIntoAttributes()) {
				...
				if (qName.equals(this._xmlAttributeRootElementName)) {
					...
					this.endCategory();
				} else this.endEntityElement(uri, localName, qName);
			}
		} catch (Throwable t) {
			ApsSystemUtils.getLogger().throwing(this.getClass().getName(), "endElement",t);
			throw new SAXException(t.getMessage(), new Exception(t));
		}		
	}
	 ...
	 */
	protected void endEntityElement(String uri, String localName, 
	...
	private void endDescr() {
		if (null != _textBuffer) {
			_currentEntity.setDescr(_textBuffer.toString());	
		}
	}
	...
	private void startGroups(Attributes attributes, String qName) throws SAXException {
		...
		_currentEntity.setMainGroup(mainGroup);
	}
	...
	private void startGroup(Attributes attributes, String qName) throws SAXException {
		...
		_currentEntity.addGroup(groupName);
	}
	...
	private void startAttribute(Attributes attributes, String qName) throws SAXException {
		if (null == _currentAttr) {
			...
			if (null != attributeName) {
				_currentAttr = (AttributeInterface) _currentEntity.getAttribute(attributeName);
				if (null != _currentAttr) {
					_parserModule = _currentAttr.getHandler();
					_parserModule.setCurrentAttr(_currentAttr);
				}
			}
		}
		if (null != _parserModule) {
			_parserModule.startAttribute(attributes, qName);
		}
	}
	
	private void endAttribute(String qName) {
		if (null != _parserModule) {
			_parserModule.endAttribute(qName, _textBuffer);
			if (_parserModule.isEndAttribute(qName)) {
				_parserModule = null;
				_currentAttr = null;
			}
		}
	...
	protected String extractXmlAttribute(Attributes attrs, String attributeName,
		...
		if(required && value == null) {
			throw new SAXException("Attributo '" + attributeName +"' assente in tag <" + qName + ">");
		}
		return value;
	}
	...
	/**
	 * @uml.property  name="_intoAttributes"
	 */
	private boolean _intoAttributes;
	/**
	 * @uml.property  name="_currentEntity"
	 * @uml.associationEnd  
	 */
	private IApsEntity _currentEntity;
	/**
	 * @uml.property  name="_parserModule"
	 * @uml.associationEnd  
	 */
	...
	/**
	 * @uml.property  name="_currentAttr"
	 * @uml.associationEnd  
	 */
